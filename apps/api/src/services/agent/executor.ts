...
type ExecutionEvent =
  ...
  | { type: "operation"; index: number; tool: string; status: "pending" | "ok" | "failed"; summary: string }
...
export async function executeBuildPlan(planId: string, sseEmit: (event: ExecutionEvent) => void) {
  ...
  try {
    ...
    const resolvedRoleIds = new Map<string, string>(); // roleKey -> discordRoleId
    const resolvedCatIds = new Map<string, string>(); // categoryKey -> discordCatId
    
    // Build logical operation list
    const ops: Array<{ tool: string; args: Record<string, any>; summary: string; context?: any }> = [];
