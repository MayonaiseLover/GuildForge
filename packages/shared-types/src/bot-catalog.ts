...
export function getBotById(botId: string): BotEntry | undefined {
  return BOT_CATALOG.find(b => b.botId === botId);
}

export function getBotsByCategory(category: string): BotEntry[] {
  return BOT_CATALOG.filter(b => b.category.toLowerCase().includes(category.toLowerCase()));
}

export function generateInviteUrl(botId: string, guildId: string): string | null {
